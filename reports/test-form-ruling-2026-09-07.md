...
## 起源 — root と request に起源は無い

Owner 指示「まずは最上流から直せ。何をするべきかが破綻しているならば直すべきは根本である」に従い、root（Issue #11 の凍結事項 F1〜F12 = `ROOT-027`〜`ROOT-038`、および裁定 `ROOT-001`〜`ROOT-026` / `ROOT-039`〜`ROOT-048`）と request（`R-1`〜`R-5`）を全数走査した。表記は「層 id（source 文書 §/行）」。層と source 文書は再層化のため一致しないことがある。

### 起源 1: 「Test を実行形態で分ける」

| | |
|---|---|
| 起源 | **require 層 `REQ-068`**（要求・要件定義 v0.1 §4.3 L165） |
| 逐語 | 「実装 construct（Source Target）を検証対象とする実行形態では、宣言された対象コードが実際に Test 実行経路へ入ったことをこの確認方法とする。」 |
| 同層の同起源 | `REQ-070` / `REQ-071`（同 §4.3 L166）、`REQ-147` / `REQ-148`（同 §9.1 L318） |

### 起源 2: 「Source Target の宣言を Test に義務づける」

| | |
|---|---|
| 起源 | **detailed_spec 層 `DS-063`**（基本仕様 v0.1 §3.3 L146） |
| 逐語 | 「1つのTestは1件以上のSource Targetを持ち、各target参照を個別に保持する。」 |
| 同起源 | `DS-480`（詳細設計 v0.1 §4.1 L519）「adapter中立coreは各管理対象Testに1件以上の検証対象を要求する。」 |

**この義務は上流に存在しない。** require 層にあるのは可能性（`REQ-144` L316「宣言**できなければならない**」）と許可（`REQ-150` L324「宣言**できる**」）だけで、spec 層も許可（`SPEC-085`）だけである。**義務は下位層の発明であり、`CLAUDE.local.md` §1.5 が「詳細設計に規則を見つけたら、上流に無ければ答えはたいてい消す」と書いている型そのものである。**

### root・request に起源が無いことの確認

語（target / Target / 実行形態 / 形態 / 宣言し / 必須 / construct / 黒箱 / 白箱）で全数走査した。root と request の該当は 3 件で、いずれも別の語義である。

| id | 該当語 | 逐語（該当部） | なぜ起源でないか |
|---|---|---|---|
| `ROOT-030`（F4） | 形態 | 「発見を可能にする **データ形態**（要求該当箇所と対応概念のペアの構造化出力）までが vtest の責務。」 | 出力の形式の話であり、Test の実行形態ではない |
| `ROOT-031`（F5） | 必須 | 「証拠ストアはハッシュキー必須。」 | 証拠ストアのキーの話であり、Test の宣言義務ではない |
| `ROOT-034`（F8） | target | 「static_audit / target_execution → 一本の **target_binding**。静的/動的は同一の問いの2つの証拠源。」 | 検査の統廃合であり、target の宣言を義務づけていない。形態にも触れていない |

`R-1`〜`R-5` に「〜を宣言しなければならない」型の義務は無い。

**したがって、退役させるべき凍結事項は無い。** team-lead の手順のうち「旧 ROOT ノードを `ROOT-049` で上書きし退役台帳に記録する」「`ROOT-049` の description に『Issue #11 の凍結事項 &lt;旧 id&gt; を改定』と書く」「Issue #11 に改定した凍結事項の id と逐語を投稿する」は、指す対象が存在しないため実施していない。実施すれば、誤っていない凍結事項を誤りとして退役させ、Issue #11 に事実でない記録を残すことになる。

Owner の指示のうち実行可能な部分（最上流から直す・上流へ差し戻す）は満たしている。`ROOT-049` を新設し、そこから require → spec → detailed_spec → basic_design → design の順に伝播させた。起源 1 の `REQ-068` は `REQ-337` へ、`REQ-147` / `REQ-070` / `REQ-071` は退役、`REQ-148` は無条件の `REQ-336` へ。起源 2 の `DS-063` は下限 0 の `DS-1673` へ。

### 伝播の経路

| 層 | 退役 | 新 id | REWORD |
|---|---|---|---|
| root | 0 | `ROOT-049`（新設） | 0 |
| request | 0（起源なし） | 0 | 0 |
| require | `REQ-070` / `REQ-071` / `REQ-132` / `REQ-147` | `REQ-336`（←`REQ-148`）/ `REQ-337`（←`REQ-068`） | `REQ-083` |
| spec | `SPEC-065` / `SPEC-301` | `SPEC-464`（←`SPEC-300`）/ `SPEC-465`（新設） | 0 |
| detailed_spec | `DS-128` / `DS-167` / `DS-176` / `DS-177` / `DS-178` / `DS-1622` / `DS-1633` / `DS-1634` / `DS-1635` | `DS-1665`〜`DS-1674`（10 件） | `DS-139` / `DS-1664`（description のみ） |
| basic_design | `BD-183` / `BD-185` / `BD-315` / `BD-316` | 0 | 0 |
| design | `DES-580` / `DES-581` | `DES-591`〜`DES-594`（4 件） | 0 |

各層について「残りの記述が直した上位層から導出できるか」を確認した結果は §3.4（そのまま残したノードと理由）と §3.5（残った齟齬の開示）にある。
...
## 4.5 未解決 — Owner へ上げるべき 1 問

**§4.4 の帰結は、Owner の裁定文そのものと衝突する可能性がある。**

Owner は「それが網羅されているものを全部通過したら合格と見做せる」と述べた。しかし現在の正本では、境界だけを検証する Test（target を宣言しない Test）を 1 件でも含む repository は合格に到達しない。連鎖は次のとおりで、すべて本適用が触れていない条文である。

1. `REQ-008` / `REQ-295` — 完全検証の OK は 4 検査すべてが `PASS` であることを要する。
2. `DS-1664` — target を持たない Test の `target_binding` は `NO_EVIDENCE`。
3. `DS-1511` — 集約は fail-closed。子に 1 つでも非 `PASS` があれば親は非 `PASS`。

→ 網羅していても合格にならない。**偽の target を書けば `PASS` に届く、という誘因が残る。** 旧仕様が偽ターゲットを 84 件生んだ圧力は、これで解消していない。

**触れていない理由**: この連鎖が当たるのは 4 検査・5 状態・`target_binding`・`DS-1664` であり、team-lead の指示が明示的に対象外としている。凍結事項 F7（`ROOT-033`）と F8（`ROOT-034`）に直結するため、PM 判断で動かすべきでない。

**Owner に確認すべきこと**: 「網羅されているものを全部通過したら合格」の「網羅」に、target を宣言しない Test の `target_binding` は含まれるか。含まれないなら、`NO_EVIDENCE` を合否集約の分母から外す規範が要る（4 検査は増えない。集約の対象範囲の問題である）。含まれるなら現状のままでよく、境界だけを検証する Test は v0.1 では合格に寄与しないことを明示する。

---

## 4.6 Issue #11 への書記コメント案（未投稿）

team-lead の指示は「改定した凍結事項の id と逐語」を Issue #11 へ投稿することだったが、**改定した凍結事項が無いため、指示どおりの本文は書けない。** 事実に合う本文の案を置く。投稿するかは team-lead の判断とする（本エージェントの権限は git と python に限られており、`gh` を実行していない）。

```
Owner 裁定 2026-09-07（Issue #14 issuecomment-5565960636）「Test を実行形態で分類しない」の
適用にあたり、起源を root（本 Issue の凍結事項 F1〜F12）から順に走査した。

結果: 凍結事項に起源は無い。改定した凍結事項は無い。

- 「Test を実行形態で分ける」の起源は require 層 REQ-068（要求・要件定義 v0.1 §4.3 L165）。
- 「Source Target の宣言を Test に義務づける」の起源は detailed_spec 層 DS-063
  （基本仕様 v0.1 §3.3 L146）。上流（REQ-144 / REQ-150 / SPEC-085）はいずれも
  「宣言できる」であって義務ではなく、義務は下位層の発明だった。
- root の該当語 3 件（F4 のデータ形態、F5 のハッシュキー必須、F8 の target_binding 統合）は
  いずれも別語義で、形態分類も宣言義務も含まない。

裁定は新 root ノード ROOT-049 として記録し、require → design へ伝播した。
正本コミット: <報告コミット hash>。詳細は reports/test-form-ruling-2026-09-07.md。

なお F7 / F8 に関わる未解決が 1 件ある（同報告書 §4.5）。target を宣言しない Test は
target_binding が NO_EVIDENCE のままとなり、REQ-295 の完全検証 OK に到達しない。
「網羅されているものを全部通過したら合格」との整合を Owner に確認したい。
```
...
| 指示 | 実施 | 理由 |
|---|---|---|
| `E-SCAN-007` を廃止 | **していない** | target 専用コードではない。§1 |
| 旧 ROOT ノードを `ROOT-049` で上書きし退役台帳へ | **していない** | root に起源が無い。誤っていない凍結事項を退役させることになる |
| `ROOT-049` の description に「凍結事項 &lt;旧 id&gt; を改定」 | **していない** | 同上。指す対象が存在しない |
| Issue #11 に書記コメントを投稿 | **していない** | 指示どおりの本文は事実でない。案を §4.6 に置いた |
| 各層 1 コミット | **していない**（5 コミット） | require と spec、basic_design と design をまとめた。退役 id を指す辺をその id を消すコミット内で付け替えるため。各コミットは単体で dangling 0。tip は push 済みで履歴の書き換えは禁止 |
| `REQ-150` / `SPEC-085` の義務を落とす | **していない** | 元から許可形。§1 |

...
`description` に PM の適用方針 (1)〜(4) を書き、適用者の開示を (5) として**別に**分けた。混ぜると root 層が「Issue コメントにそう書いてある」と主張することになるため。要旨:
...
4. 「境界形態を宣言するキー」の問い（`reports/boundary-form-observable-2026-09-07.md` §8）は消滅する。

そして (5) として、`E-SCAN-007` を退役させなかったこと（§1）を**適用者の開示**と明示して記録した。これは PM の適用方針ではない。
...
該当語を含むノードは 92 件。うち**処置したのは 43 件**である（退役 21・新 id への差し替え 19・REWORD 3。§3 の表がこの内訳。新 id 20 件のうち `SPEC-465` は掃引で当たったノードではなく新規追加なので、この 43 に含めていない）。残り 49 件は「1件以上」が VO の `derives_from`・`covers`・承認レコード件数など無関係の文脈で出たものである（`SPEC-015` / `SPEC-035` / `DS-106` / `DS-107` / `DS-209` / `DS-213` / `DS-391` / `DS-396` / `DS-801` / `DS-807` / `DS-889` / `DS-890` / `DS-1161` / `DS-1200` / `DES-117` / `DES-167` / `DES-224` / `DES-241` / `DES-336` / `DES-463` / `DES-589` ほか）。
...
**開発ゲート（`cargo fmt` / `cargo test` / `cargo clippy` / `vtest doctor`）は実行していない。** 本適用は `docs/` と `reports/` だけを変更しており Rust の成果物に触れていないため、上表の schema 適合とグラフ検査で代えた。

...
2. **ただし (a) を v0.1 で適用するかは Owner の判断事項である。** `@vtest.` の test-key 値域は閉じた列挙であり（`DS-489`、未知キーは `E-SCAN-006`）、キーを1つ足すと **7 ノード**（`DS-489` と design 層 6 件）の statement が偽になる。閉じたキー集合を広げることは v0.1 の範囲拡大にあたり、**F1（偽ターゲットの誘発を減らす）と F9（範囲拡大を避け完成を守る）が逆を向く。既定を F9 側に置き、適用していない。** 問いは §8、起草は §4・§6。
...
2. **観測子は適用していない。** §6 は起草であって適用ではない。適用の可否は F1 と F9 が逆を向く場面であるため、Owner の判断事項として §8 に 1 問で出した。
...
**境界形態の Test を宣言するキーを、v0.1 の `@vtest.` 語彙に足すか。それとも後続版へ送るか。**

「足す」場合に必要な作業は起草済みである。7 ノード（`DS-489` / `DES-215` / `DES-394` / `DES-521` / `DES-523` / `DES-524` / `DES-526`）の退役＋差し替えと、新規 2 ノードの追加になる（§6）。

### 8.2 なぜ PM が決めずに上げるか

**凍結事項の F1 と F9 が逆を向く場面だからである。**

- **F1**（守る瞬間 = AI の完了宣言の受領。救うのは嘘の `PASS` を信じてマージすること）は**「足す」**を支持する。足せば、境界形態の Test が偽のターゲットを作らされる動機が消える。旧仕様の dogfood で 84 件生じた症状がこれである。
- **F9**（v0.1 は順方向のみで出荷。**スコープ拡大は所有者の既知の悪癖であるため、完成の防衛を優先**）は**「送る」**を支持する。閉じたキー集合を広げることは v0.1 の範囲拡大にあたる。`SPEC-301`「下位仕様・**後続版へ**委譲する」の読みとも整合する。

どちらの側にも凍結事項の根拠があるため、既定は F9 の「拡大しない」に置いた。**適用していない。**

なお、**候補 (a) が (b)(c)(d) より良いこと自体は Owner 裁定を要さない**（§3 の嘘テストで決まる）。上げているのは「それを v0.1 でやるか」だけである。

### 8.3 具体例 — 同じ Test が、どちらの選択でどうなるか

例: CLI の終了コードだけを検証する Test。子プロセスとして起動し、exit code が 1 であることを assert する。内部関数を直接呼ばないので、宣言すべき内部 Source Target が無い。`REQ-148`「外部契約・境界上の振る舞いを検証する実行形態では…内部 Source Target の宣言を Test 成立性の必須条件としない」が想定している、まさにその形態である。

| | 足す | 送る（現状維持） |
|---|---|---|
| この Test の宣言 | `execution-form: external-contract` を書く。`@vtest.target` は書かない | 書けるキーが無い。`@vtest.target` を書かない |
| `E-SCAN-007` | **出ない**（宣言どおり境界形態なので `targets ≥ 1` を課さない） | **出る**（`DS-1621` の形態条件を評価できないため、実装は無条件に出す） |
| `chain_integrity` | `PASS` | `MISMATCH`（診断 `MISSING`） |
| `target_binding` | `NO_EVIDENCE`（診断 `NOT_CHECKED`。`DS-1664`） | `NO_EVIDENCE`（診断 `NOT_CHECKED`。`DS-1664`） |
| 利用者が次に取る行動 | v0.1 には確認方法が無いと分かる。宣言はそのまま置く | `E-SCAN-007` を消したくなる。**消す手段は偽のターゲットを書くことしかない** |
| 偽ターゲットを書いた場合 | 書く動機が無い | 偽ターゲットは Test 実行で通過するので `target_binding` が `PASS` になりうる。**嘘の経路が残る** |

**「送る」を選んだ場合に残る具体的な害は、この最終行である。** 偽のターゲットは `target_binding` に意味の無い `PASS` を作る。`DS-1664` はこれを塞がない。`DS-1664` が塞ぐのは target を**持たない** Test の側であり、偽でも target を持ってしまえば §7.3 の通常の合成に入るからである。

### 8.4 どちらを選んでも変わらないこと

**target を持たない Test を1件でも含む repository は、v0.1 で完全検証 OK に到達しない**（`REQ-295` / `DS-844` / `DS-1491`）。確認方法が `SPEC-301` により委譲されている以上、`target_binding` は `NO_EVIDENCE` のままだからである。委譲を正直に描画した結果であって、この選択では動かない。

### 8.5 背景にある同層の緊張（逐語。本書は解いていない）

`spec` 層の 2 ノードが食い違っている。同じ層なので層順では解けない。

> `SPEC-065`: 他の実行形態における確認方法は、当該形態に適した方法として**詳細設計で定める**。

> `SPEC-301`: 検証対象をSource Targetとして宣言しない他の実行形態（外部契約・境界上の振る舞い）の確認方法は、特定形態を他形態へ一律要求せず**下位仕様・後続版へ委譲する**。

`SPEC-065` は v0.1 の詳細設計に定める義務を課し、`SPEC-301` は後続版への先送りを許す。**「送る」を選ぶ場合、`SPEC-065` をどう満たすかが残る。** 適用済みの `DS-1664` は「確認方法とは、実施しないことである」という最も弱い読みでのみ `SPEC-065` を満たす。§7 の 1 と同じ開示である。

### 8.6 起草物の所在（すべて案。決定ではない）

| 内容 | 節 |
|---|---|
| 語彙（`execution-form` / `source-target` / `external-contract`）と `boundary` を避けた理由 | §4.3 |
| 既定値（宣言が無ければ `source-target`）と、その `REQ-135` からの導出 | §4.4 |
| `targets ≥ 1` の上流根拠（`REQ-064` ＋ `REQ-147`） | §4.2 |
| 上位層に宣言面の新設を禁じるノードが無いことの確認 | §4.1 |

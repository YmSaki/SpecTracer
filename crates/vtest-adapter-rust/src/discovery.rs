...
use thiserror::Error;
use vtest_adapter_api::AdapterDescriptor;
use vtest_model::{
    AdapterId, ContentHash, Diagnostic, ExecutionDescriptor, Locator, ScanSummary, SourceFunction,
    SourceLocation, SrcId, TargetRef, TestEntity, TestId, TestSuite, TestTarget, VoId,
};
use vtest_store::{
        ...
        let entity = TestEntity {
            ...
            execution: ExecutionDescriptor {
                ...
                suite: Some(TestSuite {
                    kind: match &test_target {
                        TestTarget::Lib => "lib",
                        TestTarget::Bin(_) => "bin",
                        TestTarget::IntegrationTest(_) => "integration",
                        TestTarget::Unknown => "unknown",
                    }
                    .to_owned(),
                    name: match &test_target {
                        TestTarget::Bin(name) | TestTarget::IntegrationTest(name) => {
                            Some(name.clone())
                        }
                        _ => None,
                    },

...
pub(crate) fn scan_project(root: &Path) -> Result<ScanResult, ScanError> {
    let config = load_config(root)?;
    scan_project_with_config(root, &config)
}

pub(crate) fn scan_project_with_adapter_config(
    root: &Path,
    adapter_config: &AdapterConfig,
) -> Result<ScanResult, ScanError> {
    let mut config = load_config(root)?;
    let includes = adapter_config
        .get("include")
        .map(|include| {
            include
                .split(',')
        ...
        .unwrap_or_else(|| config.scan.include.clone());
    let roots = adapter_config
        .get("roots")
        .map(|roots| {
            roots
                .split(',')
    ...
    config.scan.include = roots
        .iter()
        .flat_map(|root| {
            includes.iter().map(move |include| {
                if root == "." {
                    include.clone()
                } else {
                    format!("{root}/{include}")
        ...
        .collect();
    if let Some(assertion_macros) = adapter_config.get("assertion_macros") {
        config.scan.assertion_macros = assertion_macros
            .split(',')
            .filter(|value| !value.is_empty())
            .map(ToOwned::to_owned)
            .collect();
...
pub(crate) fn scan_project_with_config(
    root: &Path,
        ...
        for adapter in &config.adapters {
            if adapter.id != "rust-cargo" {
                return Err(ScanError::Adapter {
                    ...
                    message: format!("adapter `{}` is not registered", adapter.id),
                });
            }
        }
    }
...
fn record_diagnostics(
    ...
    for source in sources {
        known_ids.insert(source.target.value.clone());
...
struct Scanner<'a> {
    ...
    unregistered: Vec<SourceLocation>,
...
impl<'a> Scanner<'a> {
    ...
    fn collect_function_parts(
        ...
        let Some(annotation) = parse_annotations(attrs) else {
            self.unregistered.push(location.clone());
            self.diagnostics.push(
                Diagnostic::warning(
                    "W-SCAN-101",
        ...
        let neutral_targets = std::iter::once(&target)
            .chain(targets.iter())
            .map(|target| match target {
                TargetRef::Locator(locator) => {
                    vtest_model::NeutralTargetRef::new("rust-cargo", locator.as_string())
                }
                TargetRef::SrcId(src_id) => {
                    vtest_model::NeutralTargetRef::new("rust-cargo", src_id.as_str())
                }
            })
            .collect();
        ...
        let entity = TestEntity {
            ...
            execution: vtest_model::ExecutionDescriptor {
                ...
                suite: Some(test_suite(test_target)),
    ...
    fn finish(self, files: usize) -> Result<ScanResult, ScanError> {
        ...
        for source in &self.sources {
            *locators.entry(source.target.value.clone()).or_default() += 1;
            if let Some(src_id) = &source.src_id {
        ...
        for test in &self.tests {
            for target in &test.targets {
                let resolved = if let Some(src_id) = target.value.strip_prefix("SRC-") {
                    src_ids.get(&format!("SRC-{src_id}")).copied() == Some(1)
                } else {
                    locators.get(&target.value).copied() == Some(1)
                };
                if !resolved {
                    diagnostics.push(
                        Diagnostic::error(
...
fn test_suite(target: &TestTarget) -> TestSuite {
    let (kind, name) = match target {
        TestTarget::Lib => ("lib", None),
        TestTarget::Bin(name) => ("bin", Some(name.clone())),
        TestTarget::IntegrationTest(name) => ("integration", Some(name.clone())),
        TestTarget::Unknown => ("unknown", None),
    };
    TestSuite {
        kind: kind.to_owned(),
        name,
...
fn make_location(
    ...
    SourceLocation {
        ...
        project_relative_path: Some(relative.to_owned()),
        opaque_locator: Some(function.to_owned()),
        byte_range: Some(vtest_model::ByteRange {
            start: start_byte,
...
"#,
    ...
    fn reports_unregistered_tests() {
        ...
        assert_eq!(result.unregistered[0].function, "x");
    }

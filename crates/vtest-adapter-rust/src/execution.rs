...
pub fn run_tests(
    ...
    for test in tests {
        ...
        let observation = parse_result(&stdout, execution_selector(&test.entity));
        match observation {
...
fn cargo_command(root: &Path, test: &TestEntity) -> Command {
    ...
    command
        ...
        .arg("test")
        .arg("-p")
        .arg(execution_project(test));
    if let Some((flag, name)) = execution_target(test) {
        command.arg(flag);
        if let Some(name) = name {
            command.arg(name);
        }
    }
    command.args(["--", "--exact", execution_selector(test)]);
    command
}
...
fn cargo_llvm_cov_command(root: &Path, test: &TestEntity, output_path: &Path) -> Command {
    ...
    command
        ...
        .arg("--output-path")
        .arg(output_path)
        .args(["--", "--exact", execution_selector(test)]);
    command
}

fn command_string(test: &TestEntity) -> String {
    let target = execution_target(test)
        .map(|(flag, name)| name.map_or_else(|| flag.to_owned(), |name| format!("{flag} {name}")))
        .unwrap_or_default();
    format!(
        "cargo test -p {} {} -- --exact {}",
        execution_project(test),
        target,
        execution_selector(test)
    )
}
...
fn execution_project(test: &TestEntity) -> &str {
    test.execution
        .project
        .as_deref()
...
fn execution_selector(test: &TestEntity) -> &str {
    if test.execution.selector.is_empty() {
        &test.filter
    } else {
...
fn execution_target(test: &TestEntity) -> Option<(&'static str, Option<&str>)> {
    if let Some(suite) = &test.execution.suite {
        let target = match suite.kind.as_str() {
            "lib" => Some(("--lib", None)),
            "bin" => suite.name.as_deref().map(|name| ("--bin", Some(name))),
            "integration" => suite.name.as_deref().map(|name| ("--test", Some(name))),
            _ => None,
        };
        if target.is_some() {
            return target;
    ...
    match &test.test_target {
        TestTarget::Lib => Some(("--lib", None)),
        TestTarget::Bin(name) => Some(("--bin", Some(name))),
        TestTarget::IntegrationTest(name) => Some(("--test", Some(name))),

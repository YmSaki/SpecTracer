...
use vtest_store::{
    ...
    read_evidence, read_record_ids, read_req, read_spec, read_text, read_vo,
    required_spec_coverage_subject_keys, spec_coverage_req_sets, static_record_target_defect,
...
fn build_req_node(
    ...
    if !visiting.insert(id.to_owned()) {
        return VerificationNode {
            ...
            // §11.1: spec_coverage is the SPEC node's own item; a REQ node
    ...
    // vo_coverage item at all). spec_coverage lives on the SPEC node only.
    let items = item_copies(values, &["vo_coverage"]);
...
fn runtime_target_proof(
    ...
    let observation = record
        ...
        .targets
        .iter()
        .find(|observation| observation.target == canonical);
...
fn runtime_target_reached(
    ...
    runtime_target_proof(root, evidence, test, scan, canonical).reached
}
...
fn static_audit_item_value(
    ...
    for canonical in declared {
        let reachability = match effective_target_verdict(valid, "DA-002", canonical) {
            ...
            _ => {
                let proof = runtime_target_proof(root, evidence, test, scan, canonical);
...
mod tests {
    ...
    use vtest_store::{
        ...
        AuditReasonRecord, AuditTargetVerdictRecord, AuditorRecord, SpecRecord, SpecRef,
    };
    ...
    fn test_node_basis_distinguishes_an_audited_test_from_an_unaudited_sibling() {
        ...
        let values = items
            ...
            .map(|item| (item.item.as_str(), item))
            .collect::<BTreeMap<_, _>>();
        let audited_node = test_node(audited, &values);
        let bare = scan
            .tests
            .iter()
            .find(|test| test.id.as_str() == "TEST-BARE")
            .expect("bare test fixture");
        let bare_node = test_node(bare, &values);

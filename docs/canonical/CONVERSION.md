...
## 10. 既知の欠陥: 章見出しの一律ノード化と表の粒度の不一致（2026-09-08）

変換は md の章見出しを層ごとに一律ノード化し、見出し1つにつき節ノード（`title` のみを持ち `statement` を持たない）を1つ生成する。一方、導出表・トレーサビリティ表は下位節（子見出し・条文）の粒度でしか書かれておらず、親見出し自身の行が存在しない文書がある。この2つの生成規則の不一致により、直下に規範文（statement を持つ子）を1つも持たない見出しノードが混入する（`orphans-2026-09-08.md` で72件検出、Owner裁定によりそのうち削除可能な件は撤去済み。`relations/retired-ids.json` を参照）。

変換規則自体（見出しの一律ノード化）は今後も維持する。表の粒度を見出し粒度に合わせて書き直すか、見出しノードの生成基準を変えるかは、変換規則の改訂であり本書の管轄だが、2026-09-08 時点では未着手。新しい章立ての文書を変換する際は、親見出し自身に対応する表の行が無いまま統合すると同じ欠陥が再発する点に注意する。

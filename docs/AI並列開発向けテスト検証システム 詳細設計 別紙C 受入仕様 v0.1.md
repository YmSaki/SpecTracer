...
- Source Targetはcanonical Target Referenceと任意の恒久SRC IDを併有する単一のentityである。adapterは同一constructをlocator版とSrcId版の2 draftへ複製せず、恒久SRC IDを`SourceTargetDraft.src_id`として返す。
...
- 恒久SRC IDはSource Target hashのinputに含めない。

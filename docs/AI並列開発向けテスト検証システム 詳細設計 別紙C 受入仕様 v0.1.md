...
- Target Reference解決は解決済み / 対象なし / 曖昧を区別し、曖昧はfail-closedな終端状態とする。E-SCAN-004またはE-SCAN-011で曖昧・未解決となったtargetについて、Source Target具体化、監査subject、Evidence、`target_execution`のいずれも候補の1件を選んで記録しない。候補は診断表示にだけ用いる。

...
詳細設計は本冊（コア設計 §1〜§11、§16〜§17、§19）、別紙A（CLI・MCPインターフェース仕様 §12〜§15）、別紙B（実装計画 §18）の3分冊であり、節番号は全冊を通した連番である。本書からの「詳細設計 §n」の参照はこの連番を指す。

本書と詳細設計を入力として、実装エージェント（Codex）が実装を行うことを想定する。
要件定義 §34 で持ち越された未確定事項18項目は、本書および詳細設計で解消する（解消状況は §15 参照）。

本書で確定する前提は次のとおり。

- **ツール名**：`vtest`（仮称。バイナリ名・ディレクトリ名に使用する）
- **実装言語**：Rust
- **初期対象**：Rust の関数単体テストおよび小規模な結合テスト（`#[test]` 属性を持つテスト関数）
- **インターフェース**：CLI と、AI Agent 向け MCP サーバ
- **AI監査の実行方式**：エージェント委譲方式を主経路とする。`vtest` 自身は LLM API を呼ばず、監査に必要な情報を構造化して出力し、外部のAIエージェントが判定結果を構造化して提出する。将来のAPI直接呼び出しは拡張点として確保する（§7.3）
...
将来、rename 耐性が必要になった箇所だけ、ソース側アノテーション（`@vtest.src-id SRC-...`）による恒久ID付与へ昇格できる。
昇格は初期リリースの必須機能としない。
...
`STALE` は要件定義 §15「対象実装変更前に得られたPASSを、無条件に現在のPASSとして利用してはならない」を状態として明示するために追加した値である。
...
**API直接呼び出しの拡張点**：将来、`vtest` が LLM API を直接呼ぶモードを追加する場合も、上記のバンドル生成・結果検証・保存の経路を共用し、「判定」の実行主体だけを内部化する。初期リリースでは実装しない。
...
満たさない Evidence は `STALE` とし、有効な PASS として扱わない。
...
初期リリースには Rust 関数単体テスト用と小規模結合テスト用の組込スキーマを同梱する。
...
既存プロジェクトには段階的に導入できる（要件定義 §23）。

```text
Phase 0  vtest init
         .verify/ を作成。既存コードは変更しない。

Phase 1  vtest scan
         既存 #[test] 関数を unregistered test として一覧化。
         現状の可視化のみ。

Phase 2  SPEC / REQ / VO の登録
         仕様文書を SPEC として登録し、REQ・VO を作成する。
         flat な VO 群として登録してよい（階層化は後から可能。
         要件定義 §4.3）。

Phase 3  既存 Test の登録
         既存テストへアノテーションを付与し、VO と対応付ける。
         Structured Edit または直接編集のどちらでもよい。

Phase 4  監査・実行・完全検証
         決定論的監査、意味監査、実行検証を段階的に有効化する。
```

各 Phase の途中でも `vtest verify` は実行でき、未登録・未検証の部分は fail-closed により非 PASS として可視化される。
「導入が済んでいない」ことと「検証済みで OK」が混同されることはない。
...
| 7 | AST/LSP 等の解析技術 | 詳細設計 §5：syn による AST 解析。LSP は初期対象外 |
...
| 14 | キャッシュ / index のデータ形式 | 詳細設計 §2：初期リリースは実行毎のインメモリ再構築とし、永続キャッシュは任意最適化 |
| 15 | 並列編集時の物理的保存方式 | 本書 §5.2 |
| 16 | 境界値・partition を必須入力にする Test 種別 | 初期リリースでは必須とする種別を設けない。Form Schema の `required` で将来指定可能（詳細設計 §14） |

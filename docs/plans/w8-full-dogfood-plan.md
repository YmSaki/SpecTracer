...
- **covers 設計 完了・適用前で停止**（docs/reports/w8-covers-proposal.md + docs/plans/dogfood-covers-proposal.json）: rule =「covers = correspondence = PROVES|PARTIAL」（v3 基準）。169 tests → 385 mappings、**auxiliary（covers ゼロ）23** = B 19 + C 2 + A棄却 2。**destructive remap の適用は auxiliary の Owner spec 判断（免除種別 / 未管理化 / MISSING 受容）でブロック** — 判断後は proposal → 注釈書換 → doctor → 旧63/7 retire まで一気に実行可能。

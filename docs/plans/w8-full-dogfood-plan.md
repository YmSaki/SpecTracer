...
- **branch**: feature/adapter-separation-alpha2-implementation @ 525e6b9（wave1+2+3 merge 済み・gate 緑）。
- **wave 3 完了（2026-08-17）**: 3 worker 全 DONE。cluster 1（spec-coverage end-to-end: evaluate_spec_coverage + 縮退 matrix 6状態 spec 引用付き・E-AUDIT-007・SPEC tree node・ScopeSelection.spec_ids 拡張・旧 REQ→VO linkage evaluator 退役・TEST-CLI-077 は偽 PASS を主張していたため正規 bundle 提出へ修正）/ cluster 2（E-SCAN-012 emitter + vo_decomposition の subject-routed 化・adapter parse error の隔離を TEST-CLI-120 で固定・VO-PLAN-01 は副産物で閉包 = not-needed）/ cluster 5（REQ-anchored vo_coverage・zero-VO active REQ → MISSING・withdrawn 除外・wave2 recency fold + approval gate 保存・TEST-VERIFY-023 は vo_coverage_audit_fold へ再照準）。merge 衝突は全て事前規則（両意図の和集合）で解決: REQ node は vo_coverage のみ携行（spec_coverage は SPEC node 所有）、ScopeSelection literal 7件に spec_ids 補完。統合 gate: cargo test --workspace 全 suite ok / clippy 0 warnings。**root cause 進捗: 14/19**（残 = cluster 6・8・9 + VO-EXIST-08 + VO-INTAKE-04 = wave 4 で 19/19）。
- **wave 4 完了・merge 済み（2026-08-17, feature @ 46b163d）— 19/19 root cause 修正完遂**: cluster 6（create/edit_test へ registry 貫通・Form kind → 単一 StructuredTest adapter 解決〔0/2+ fail-closed〕・render_edit を adapter trait へ移設 = `/** */` 根治・accepts_compatibility_form / built_in_form_kinds に初の呼び出し・form_get 同一ゲート）/ cluster 8 + 2 singleton（evaluate_target_execution を検証済み entry set 導出へ・per-target report children・**VO-EXIST-08: Scanner::finish が Complete を hardcode していた事実を worker が発見し併せて修正**・VO-INTAKE-04 state-anchored 化 = spec-audit-subject **v1→v2 bump**〔既存 "spec" subject 監査は再 bundle まで STALE — 意図的・前例 STATIC_AUDIT_RULE_SET_VERSION〕・回復アサーション②が base の event-anchored 欠陥を実測で裏付け）/ cluster 9（Annex A detail report・Project checks 節・kind 非依存 children 描画・per-Test basis narrowing・VO-AGG-10 runtime proof citation）。B↔C 衝突は設計統合で解決: runtime_target_proof が C の citation を持ちつつ B の resolve_target_execution_entries 経由で導出（entry-set 述語は単一）。統合 gate: cargo test --workspace 全 suite ok / clippy 0 warnings。
- **残渣メモ（fix 対象外・記録）**: vtest-store records.rs の filter_map が target 欠落 target_execution entry を parse 時に黙って落とす（既存挙動・worker B 申告）。remap 前 hygiene 候補。
- **wave 4 後の repo-root scan 再観測（46b163d）**: W-SCAN-102 ×131（143→131 = 新テストの covers による自然減）/ W-SCAN-101 ×25 / E-SCAN-002 ×1（既知 TEST-CLI-098）。新規エラー種なし・.verify 非改変。
- **④ repo-root scan 観測（525e6b9, 記録のみ・弱める修正禁止）**: NG。summary = files 30 / sources 1122 / tests 245。内訳: **W-SCAN-102 ×143**（旧 registry の未 cover VO — retire/remap で解消する debt の可視化）/ W-SCAN-101 ×25（無注釈 test）/ **E-SCAN-002 ×1**（TEST-CLI-098 重複 — vtest-cli/src/lib.rs と tests/m1_acceptance.rs。base 65c4125 に既在で merge 起因でない。covers 参照があるため remap 時に片方を改番）。E-SCAN-012 は 0件（旧 registry の spec_refs に dangling なし）。.verify 非改変を確認済み。

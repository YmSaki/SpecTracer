...
- **次**: role 仕様 PR（③同梱・syntax 確定）→ remap 適用 → doctor → 旧63/7 retire。数値正典: **171 VO**、検証済み CONFIRMED は **40**（39 + STRUCTOP-15。ほか scope 非依存副次 = E-SCAN-005 握り潰し、REGISTRY-21 候補は cluster 4 fix に随伴）。

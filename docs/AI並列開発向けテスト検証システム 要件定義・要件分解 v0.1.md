...
## 11.1 仕様適合性の証拠としての成立条件

仕様適合性の証拠として算入するTestは、検証対象の振る舞いがそのTestの成否に因果的に反映されるものでなければならない。

この上位原則の必要条件として、以下を要求する。

1. **検証成立性**：仕様適合性の証拠として算入するTestは、検証対象の振る舞いを反映した結果・状態・その他の観測に基づいて期待条件への適合と不適合を識別し、不適合がTestの非成功として反映されるものでなければならない。
2. **依存要素の信頼性**：Testの適合性判定が他の構成要素の能力に依存する場合、その依存要素の正当性が確認されていない状態で、そのTestを有効な仕様適合性の証拠として扱ってはならない。

## 11.2 成立性確認の原則

3. **証明方法への非依存**：成立条件の確認方法は、検証対象、実行形態、観測方法に応じて異なってよい。特定の実行形態に固有の確認方法を、別の実行形態へ一律に要求してはならない。
4. **未確認と違反の区別**：成立条件を確認できないことと、成立条件に違反していることを区別しなければならない。確認不能であることだけを根拠として違反を推定してはならない。

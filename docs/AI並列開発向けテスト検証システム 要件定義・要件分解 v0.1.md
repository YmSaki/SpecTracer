...
管理対象となるTestは、その宣言された目的に対して、検証対象の振る舞いを反映した観測に基づく有効な成否判定を持たなければならない。Testの成否判定は、検証対象の振る舞いを反映した観測に依存しなければならない。

そのうえで、仕様適合性の証拠として算入するTestは、その検証成立性が確認済みでなければならない。Testとして成立しているかの検査と、仕様適合性の証拠として算入するかの判定は独立である。

この成立性の必要条件として、以下を要求する。

1. **検証成立性**：Testは、検証対象の振る舞いを反映した結果・状態・その他の観測に基づいて期待条件への適合と不適合を識別し、不適合がTestの非成功として反映されるものでなければならない。
2. **依存要素の信頼性**：Testの成否判定が他の構成要素の判定能力に依存する場合、その依存要素について正当性が確認されるか、検証基盤として明示的に信頼されていなければ、当該Testの検証成立性を確認済みとして扱ってはならない。判定能力を担う依存要素は、正当性確認対象または明示的な信頼基盤として識別可能であり、成立性確認はそのいずれかで終端しなければならない。

## 11.2 成立性確認の原則

3. **証明方法への非依存**：成立条件の確認方法は、検証対象、実行形態、観測方法に応じて異なってよい。特定の実行形態に固有の確認方法を、別の実行形態へ一律に要求してはならない。
4. **未確認と違反の区別**：成立条件を確認できないことと、成立条件に違反していることを区別しなければならない。確認不能であることだけを根拠として違反を推定してはならず、成立確認済みとして扱ってもならない。

成立条件から明確に外れるTestは、可能な限り決定論的な解析によって検出する。
決定論的解析で明確に不成立とできるTestは、LLM監査へ送る前に拒否可能であること。

## 11.3 決定論的に検出可能な不成立構造

以下は、§11.1の成立条件を満たさないことを決定論的に検出できる具体例である。code fragmentはRustによる例示であり、各adapterは対応する言語・runnerの構造に対して決定論的に判定できる範囲を提供する。

```rust
assert!(true);
```

Testの実行が、そのTestが検証対象とする振る舞いを生じさせていない（Rust white-box方式の例: 宣言されたSource Targetを実行していない）。
...
検証対象の振る舞いを生じさせるだけで、その結果・状態・観測を成否判定に利用していない。
...
その他、同様にTestの成否判定が検証対象の振る舞いを反映した観測に依存しない構造。
...
> そのTestが検証対象とする振る舞いが実際に生じ、その振る舞いを反映した観測が得られたこと

を、当該Testの実行形態に適した方法で確認できなければならない。

TestがPASSしても、検証対象とする振る舞いを実際には生じさせていない場合、そのTestを完全検証済みOKとしない。

Source Target（実装construct）を検証対象とする実行形態では、宣言された対象コードが実際にTest実行経路へ入ったことをこの確認方法とする。この形態で複数targetを宣言したTestでは、各targetの実行を個別に計測する。1件でも実行回数が0なら`FAIL`、1件でも解析不能でかつ`FAIL`がなければ`UNKNOWN`とし、全targetの実行を確認できた場合だけ`PASS`とする。
他の実行形態における確認方法は、§11.2の証明方法への非依存に従い、当該形態に適した方法として下位仕様で定める。特定の実行形態の確認方法を、別の実行形態のTestへ一律に要求してはならない。
...
`Target execution` は §16 の確認（検証対象の振る舞いの実現）を指す。その確認方法は当該Testの実行形態に適したものでよく、特定の実行形態の確認方法をこの項目の名の下に全Testへ一律に要求してはならない。

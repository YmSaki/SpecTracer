...
2. **依存要素の信頼性**：Testの適合性判定が他の構成要素の能力に依存する場合、その依存要素の正当性が確認されていない状態で、そのTestを有効な仕様適合性の証拠として扱ってはならない。判定能力を担う依存要素については、正当性を確認する対象と、検証基盤として明示的に信頼する対象とを区別できなければならず、成立性の確認はそのいずれかで終端しなければならない。
